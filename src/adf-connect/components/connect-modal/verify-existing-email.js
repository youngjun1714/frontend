...
const VerifyExistingEmail = (props) => {
  const { code, onChangeCode, email, setIsEmailVerified } = props;

  const { openModalPage, setAccount, setTempToken } = useConnectModalContext();
  ...
  const handleSubmitVerifyCode = async () => {
    try {
      ...
      const response = await axios.post(`/join/verify-exist-mail`, data);
...
export default VerifyExistingEmail;
